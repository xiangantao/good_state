...
class AutoencoderKLWan(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def _decode(self, z: torch.Tensor, return_dict: bool = True):
        ...
        # Decode each frame independently without cache to maintain consistent quality
        frame_outputs = []
        for i in range(num_frame):
            self.clear_cache()  # Clear cache for each frame to treat them equally
            self._conv_idx = [0]
            frame_out = self.decoder(x[:, :, i : i + 1, :, :], feat_cache=self._feat_map, feat_idx=self._conv_idx)
            frame_outputs.append(frame_out)
        
        out = torch.cat(frame_outputs, dim=2)
