...
下游固定表更新：已确定保留 256 维后，在全部五个校准场景上共同拟合[一份通用的固定通道表](../../scannet_channels/73f4e810824b58cb/global_256/report.md)，所有视频复用同一组编号，不再选择 `held_out`。本文原有五折结果保持原义，不作为这份固定表的独立测试成绩。
